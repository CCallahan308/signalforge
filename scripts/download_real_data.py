...
class RealDataIngestion:
    ...
    def download_and_process(self, dataset_key: str) -> pd.DataFrame:
        ...
        # The feature pipeline (engineer_real_features.py) reads the raw IBM
        # Telco CSV from a fixed path. Persist the unmodified raw frame there
        # before any processing so the documented quickstart runs end-to-end.
        if dataset_key == 'telco':
            raw_out = self.data_dir / 'WA_Fn_UseC_Telco_Customer_Churn.csv'
            df.to_csv(raw_out, index=False)
            logger.success(f"Saved raw Telco CSV for the feature pipeline: {raw_out}")
